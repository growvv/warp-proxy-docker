...
RUN ARCH=$(uname -m) && \
    if [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ]; then \
        WARP_GO_URL="https://gitlab.com/ProjectWARP/warp-go/-/raw/master/warp-go_1.0.8_linux_arm64.tar.gz"; \
        WP_URL="https://github.com/windtf/wireproxy/releases/download/v1.1.2/wireproxy_linux_arm64.tar.gz"; \
    elif [ "$ARCH" = "x86_64" ] || [ "$ARCH" = "amd64" ]; then \
        WARP_GO_URL="https://gitlab.com/ProjectWARP/warp-go/-/raw/master/warp-go_1.0.8_linux_amd64.tar.gz"; \
        WP_URL="https://github.com/windtf/wireproxy/releases/download/v1.1.2/wireproxy_linux_amd64.tar.gz"; \
    else \
        echo "Unsupported architecture: $ARCH" && exit 1; \
    fi && \
    curl -L -o /tmp/warp-go.tar.gz $WARP_GO_URL && \
